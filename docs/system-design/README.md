...
| 7 | [`ai-platform-topology.html`](ai-platform-topology.html) | **The AI platform layer** (ADR 0008): LiteLLM gateway as the one LLM egress hop — two provider routes, Claude Haiku primary with OpenAI as its fallback — plus Langfuse tracing, pgvector RAG, and Argo (orchestration fork open — see the doc) as private-subnet workloads, and a CI eval gate in GitHub Actions. See [`ai-platform.md`](ai-platform.md) for the full design |
